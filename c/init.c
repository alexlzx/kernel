...
void initproc( void )                /* The beginning */
{
    ...
    // -------------------------------- new added by Alex --------------------------------
    // initialize device table for 2 devices
    int i;
    for (i=0;i<DEVICE_TABLE_SIZE; i++){
        devtab[i].dvnum = KBMON;
        devtab[i].dvopen = (int (*)(void*, int)) &kbd_open;
        devtab[i].dvclose = (int (*)(void*, int)) &kbd_close;
        devtab[i].dvwrite =  (int (*)(void*, int)) &kbd_write;
        devtab[i].dvread = (int (*)(int, void*, int)) &kbd_read;
        devtab[i].dvcntl = (int (*)(unsigned long, unsigned int)) &kbd_ioctl;
