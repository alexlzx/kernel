...
static int  kill(pcb *currP, int pid, int signalNumber);
    ...
    for( p = next(); p; ) {
        ...
        switch( r ) {
            ...
            case( SYS_SIGHANDLER ):
                ap = (va_list)p->args;
                signalNumber = va_arg( ap, int );
                newhandler = (va_arg( ap, void (*) (void*)));
                oldHandler = (va_arg( ap, void (**) (void*)));
                p->ret = signalhandler(p, signalNumber, newhandler, oldHandler);
                break;
            case( SYS_RETURN ):
                ap = (va_list)p->args;
            ...
            case ( SYS_IOCTL ):
                ...
                ioctl_args = va_arg(ap, int);
                p->ret = di_ioctl(p, fd, command, ioctl_args);
                break;
...
static int kill(pcb *currP, int pid, int signalNumber) {
    ...
    return signal(pid, signalNumber);
}
